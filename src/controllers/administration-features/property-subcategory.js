        ...
        try {
            ...
            await knex.transaction(async (trx) => {
               ...
               // console.log('[controllers][category][add]: CategoryCode', existCategoryCode);
                ...
                const insertData = { ...subcategoryPayload,  isActive: 'true', createdAt: currentTime, updatedAt: currentTime };

                console.log('[controllers][subcategory][add]: Insert Data', insertData);

                const incidentResult = await knex.insert(insertData).returning(['*']).transacting(trx).into('incident_sub_categories');
    ...
    updateSubCategory: async (req, res) => {
        ...
        try {
            ...
            await knex.transaction(async (trx) => {
                ...
                console.log('[controllers][SubCategory][updateSubCategory]', subCategoryPayload);
                ...
                const result = Joi.validate(subCategoryPayload, schema);
                console.log('[controllers][SubCategory][updateSubCategory]: JOi Result', result);
                ...
                const updateDataResult = await knex.update({ descriptionEng : subCategoryPayload.descriptionEng, descriptionThai : subCategoryPayload.descriptionThai, updatedAt : currentTime }).where({ id: subCategoryPayload.id }).returning(['*']).transacting(trx).into('incident_sub_categories');
               ...
               // const updateData = { ...incidentTypePayload, typeCode: incidentTypePayload.typeCode.toUpperCase(), isActive: 'true', createdAt: currentTime, updatedAt: currentTime };
                ...
                console.log('[controllers][SubCategory][updateSubCategory]: Update Data', updateDataResult);
    ...
    deleteSubCategory: async (req, res) => {
        ...
        try {
            ...
            await knex.transaction(async (trx) => {
                ...
                console.log('[controllers][Subcategory][subcategoryDelete]', subCategoryPayload);
                ...
                const result = Joi.validate(subCategoryPayload, schema);
                console.log('[controllers][Subcategory][subcategoryDelete]: JOi Result', result);
                ...
                const notexistSubCategory = await knex('incident_sub_categories').where({ id: subCategoryPayload.id });
              
                console.log('[controllers][Subcategory][subcategoryDelete]: SubCategoryId', notexistSubCategory);
                ...
                if (notexistSubCategory == "") {
                    return res.status(400).json({
                        errors: [
                            { code: 'TYPE_CODE_EXIST_ERROR', message: 'SubCategory does not exist !' }
                        ],
                    });
                }           
                ...
                //const updateDataResult = await knex.table('incident_type').where({ id: incidentTypePayload.id }).update({ ...incidentTypePayload }).transacting(trx);
                const updateDataResult = await knex.update({ isActive : 'false', updatedAt : currentTime }).where({ id: subCategoryPayload.id }).returning(['*']).transacting(trx).into('incident_sub_categories');
    ...
    subCategoryList: async (req, res) => {
        ...
        try {
            ...
            await knex.transaction(async (trx) => {                        
                ...
                const DataResult = await knex('incident_sub_categories').where({ isActive: 'true' });               
            ...
            listSubCategories = await Parallel.map(listSubCategories, async item => {
                let incidentCategory = await knex('incident_categories').where({ id: item.incidentCategoryId }).select('*');
                item.incidentCategoryDetail = incidentCategory[0];
                delete item.incidentCategoryId;


                let incidentType = await knex('incident_type').where({ id: item.incidentTypeId }).select('*');
