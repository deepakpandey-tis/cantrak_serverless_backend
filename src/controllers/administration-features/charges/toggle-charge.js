...
const toggleCharge = async (req, res) => {
    try {
        ...
        console.log(
            "[controllers][administration-features][charges]toggleCharge: JOi Result",
            result
        );
        ...
        await knex.transaction(async (trx) => {
            ...
            let check = await knex('charges').select('isActive').where({ id: payload.id, orgId: orgId }).first();
            if (check.isActive) {
                sqlResult = await knex
                    ...
                    .transacting(trx)
                    .into('charges');
                message = "Charge de-activated successfully!"
            } else {
                sqlResult = await knex
        ...
        console.log("[controllers][administration-features][charges][toggleCharge] :  Error", err);
        //trx.rollback
        res.status(500).json({
