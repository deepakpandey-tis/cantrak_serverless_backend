            ...
            let [total, rows] = await Promise.all([
                ...
                .where(qb => {
                    ...
                    if (req.body.workOrderDate) {
                        ...
                        // console.log("==============", workDateFrom, "=============", "w", workDateTo, "================")

                        qb.whereBetween('task_group_schedule_assign_assets.pmDate', [workDateFrom, workDateTo])


                        //qb.where('task_group_schedule_assign_assets.pmDate', period)
                        //qb.whereRaw(`to_char(to_timestamp(task_group_schedule_assign_assets."pmDate"/1000),'YYYY-MM-DD')='${period}'`)
                        //  qb.whereRaw(`task_group_schedule_assign_assets."pmDate" = "2020-09-16"`)
                        //  qb.whereRaw(`Math.round(task_group_schedule_assign_assets."pmDate".getTime()/1000) = 2020-09-16`)
