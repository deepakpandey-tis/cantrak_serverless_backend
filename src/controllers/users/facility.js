...
const facilityBookingController = {
    ...
    try {
      ...
      resultData = await knex
        ...
        .where({
          ...
          "parcel_management.pickedUpType": parcelType,
        })
      ...
      totalNewParcel = await knex
        ...
        .where({
          "parcel_management.orgId": req.orgId,
          "parcel_management.pickedUpType": parcelType,
          "parcel_management.parcelViewStatus": "1",
        })
        .first();

      console.log("totalUnreadParcel", totalNewParcel);

      let totalNewParcelAdded =
        totalNewParcel.totalNewAddParcel;
      console.log(
        "totalNewParcelAdded",
        totalNewParcelAdded
      );
      ...
      await Parallel.map(totalNewParcelList, async (up) => {
        // Update view status as view all outgoing parcel by user
        const updateParcelStatus = await knex(
          "parcel_management"
        )
  ...
  getPickedUpParcelList: async (req, res) => {
    try {
      ...
      const Parallel = require("async-parallel");
      resultPickUp = await Parallel.map(
        resultPickUp,
        async (pp) => {
  ...
  getUserPendingApprovalList: async (req, res) => {
    try {
      ...
      resultData = await knex
        ...
        .where({ "parcel_management.orgId": req.orgId })
        .where({
          "parcel_management.isPendingForApproval": true,
        });
    ...
    try {
      ...
      resultData = await knex
        ...
        .where((qb) => {
          ...
          if (startDateTime && endDateTime) {
            qb.where(
              "entity_open_close_times.openTime",
              ">=",
              startDateTime
            );
            qb.where(
              "entity_open_close_times.closeTime",
              "<=",
              endDateTime
            );
          }
      ...
      resultData = await Parallel.map(
        ...
        async (pd) => {
          ...
          let bookingDay =
            moment(currentTime).format("ddd");
          console.log(
            "Checking Booking Availability of Day: ",
            bookingDay
          );
          ...
          let iconResult = await knex
            .from("images")
            .select("s3Url", "title", "name")
            .where({
              entityId: pd.facilityTypeId,
              entityType: "facility_type_icon",
            })
            .first();

          let startDate = moment(currentTime)
            .startOf("date")
            .format();
          let endDate = moment(currentTime)
            .endOf("date")
            .format();
          ...
          let checkMaxBooking = await knex(
            "entity_bookings"
          ).where({
            entityId: pd.id,
            entityType: "facility_master",
  ...
  userFacilityDetails: async (req, res) => {
    try {
      ...
      console.log("User details", req.me);
      ...
      let [
        ...
        await knex
          ...
          .first(),
        knex.from("entity_open_close_times").where({
          entityId: payload.id,
          entityType: "facility_master",
          orgId: req.orgId,
        }),
        knex.from("rules_and_regulations").where({
          entityId: payload.id,
          entityType: "facility_master",
          orgId: req.orgId,
        }),
        ...
        knex("entity_booking_limit")
          .select(["limitType", "limitValue"])
          .where({
            entityId: payload.id,
            entityType: "facility_master",
            orgId: req.orgId,
        ...
        knex("facility_property_unit_type_quota_limit")
          ...
          .where({
            "facility_property_unit_type_quota_limit.entityId":
              payload.id,
            "facility_property_unit_type_quota_limit.entityType":
              "facility_master",
            "facility_property_unit_type_quota_limit.orgId":
              req.orgId,
          }),
        knex
          .from("organisations")
          .select(["organisations.configJson"])
          .where({ "organisations.id": req.orgId })
          .first(),
      ]);
      ...
      let iconResult = await knex
        ...
        .select("s3Url", "title", "name")
        .where({
          entityId: facilityDetails.facilityTypeId,
          entityType: "facility_type_icon",
        })
        .first();
  ...
  yourFacilityBookingList: async (req, res) => {
    try {
      ...
      totalNewBookingsData = await knex
        .from("entity_bookings")
        .where(
          "entity_bookings.bookingEndDateTime",
          ">=",
        ...
        .where({
          "entity_bookings.bookedBy": id,
          "entity_bookings.viewStatus": true,
        })
        .count("* as totalNewBookings")
        .where({ "entity_bookings.orgId": req.orgId })
        .first();

      let totalUpcomingBookings =
        totalNewBookingsData.totalNewBookings;
      ...
      resultData = await Parallel.map(
        ...
        async (pd) => {
          ...
          console.log(
            "checkedDate",
            pd.bookingStartDateTime
          );
          let checkBookedDate;
          if (pd.bookingStartDateTime) {
            checkBookedDate = moment(
              pd.bookingStartDateTime,
              "x"
            ).format("YYYY-MM-DD");
          }
          ...
          let resultToday =
            moment(checkBookedDate).isSame(time); // true
          ...
          let resultTomorrow =
            moment(checkBookedDate).isSame(tomorrow); // true
      ...
      await Parallel.map(resultData, async (up) => {
        // Update view status as view all outgoing parcel by user
        const updateBookingStatus = await knex(
          "entity_bookings"
        )
  ...
  facilityBookNow: async (req, res) => {
    try {
      ...
      // Check booking Quota
      let checkFacilityQuota =
        await facilityHelper.getBookingQuota({
      ...
      if (
        checkFacilityQuota.code &&
        checkFacilityQuota.message
      ) {
        return res.status(400).json({
          errors: [
            {
      ...
      let checkFacilityCapacity =
        await facilityHelper.getBookingCapacity({
          facilityId: payload.facilityId,
          bookingStartDateTime:
            payload.bookingStartDateTime,
          bookingEndDateTime: payload.bookingEndDateTime,
      ...
      if (closeFacility) {
        let closeReasonMessage =
          closeFacility.inActiveReason;
      ...
      let closeFacilityTiming = await knex(
        "facility_close_date"
      )
        .select("*")
        .where({
        ...
        .where(
          "facility_close_date.endDate",
          ">",
          payload.bookingStartDateTime
        )
        .where(
          "facility_close_date.startDate",
          "<",
      ...
      let bookingCriteria1 = await knex(
      ...
      let totalConcurrentLimit =
        bookingCriteria1.concurrentBookingLimit;
      ...
      if (
        ...
        allowBookingSeat =
          Number(1) + Number(bookingData.totalBookedSeats);
      ...
      if (price.feesType == "1") {
        ...
        let calDuration =
          (facilitySlot.slotDuration * price.feesAmount) /
          price.duration;
      ...
      if (facilityData.bookingStatus == "2") {
        ...
        let projectId = await knex("property_units")
          .select("projectId")
          .where({ id: payload.unitId, orgId: req.orgId })
          .first();
        ...
        userTeamResult = await accessRoleHelper.getAllUsers(
          {
        ...
        if (orgAdminResult) {
          let adminUser = await knex("users")
            .select("*")
            .where({
              id: orgAdminResult.organisationAdminId,
            })
            .first();
  ...
  getFacilityAvailableSeats: async (req, res) => {
    try {
      ...
      let facilityData = await knex
        ...
        .where({
          "entity_booking_criteria.entityId":
            payload.facilityId,
          "entity_booking_criteria.entityType":
            "facility_master",
      ...
      let bookingDay = moment(bookingStartTime).format(
        "ddd"
      );
      console.log(
        "Checking Booking Availability of Day: ",
      ...
      let bookingFullDay = moment(bookingStartTime).format(
        "dddd"
      );
      ...
      let bookingCriteria = await knex(
        "entity_booking_criteria"
      )
        .select("*")
        .where({
      ...
      if (
        ...
        let openingTimeOnBookingDay = moment(
          bookingStartTime
        )
          .hours(moment(+openCloseTimes.openTime).hours())
          .minutes(
            moment(+openCloseTimes.openTime).minutes()
          )
          .seconds(0)
          .milliseconds(0);

        let closingTimeOnBookingDay = moment(
          bookingStartTime
        )
          .hours(moment(+openCloseTimes.closeTime).hours())
          .minutes(
            moment(+openCloseTimes.closeTime).minutes()
          )
          .seconds(0)
          .milliseconds(0);
        ...
        console.log(
          ...
          openingTimeOnBookingDay.format(
            "YYYY-MM-DD HH:mm:ss"
          )
        );
        console.log(
          "closingTimeOnBookingDay:",
          closingTimeOnBookingDay.format(
            "YYYY-MM-DD HH:mm:ss"
          )
        );

        if (
          openingTimeOnBookingDay.valueOf() >
          moment(bookingStartTime).valueOf()
        ) {
          return res.status(400).json({
            errors: [
              {
        ...
        if (
          closingTimeOnBookingDay.valueOf() <
          moment(bookingEndTime).valueOf()
        ) {
          return res.status(400).json({
            errors: [
              {
        ...
        let bookingPeriodAllow = await knex(
          "entity_booking_criteria"
        )
        ...
        if (
          bookingPeriodAllow &&
          bookingPeriodAllow.minBookingPeriod
        ) {
          minDuration =
            moment(+payload.bookingEndDateTime) -
            moment(+payload.bookingStartDateTime);
          let minDurationInMinutes =
            minDuration / 1000 / 60;
      ...
      let bookingAllowingTiming = await knex(
        "entity_booking_criteria"
      )
        .select([
      ...
      console.log(
        "Booking Start Time:",
        moment(bookingStartTime).format(
      ...
      if (
        ...
        if (!isValidBookingInsideAllowPeriod) {
          ...
          if (parseInt(advanceString / 24 / 60) > 0) {
            //  advanceString = parseInt(advanceString / 24 / 60) + " days, " + parseInt(advanceString / 60 % 24) + ' hours, ' + parseInt(advanceString % 60) + ' minutes';
            advanceString =
              parseInt(advanceString / 24 / 60) + " days";
          } else {
            advanceString =
            ...
            errors: [
              {
                ...
                message: `Booking needs to be made before ${bookingAllowingTiming.bookingAllowedAdvanceTime} minutes of booking start period.`,
              },
            ],
          });
        }
      ...
      if (dailyQuota && dailyQuota > 0) {
        ...
        let totalBookedSeatForADay =
          rawQuery.rows[0].totalseats;
        console.log(
          "total Bookings Done for a day",
          totalBookedSeatForADay
        );
      ...
      if (weeklyQuota && weeklyQuota > 0) {
        ...
        let totalBookedSeatForAWeek =
          rawQuery.rows[0].totalseats;
        console.log(
          "total Bookings Done for a week",
          totalBookedSeatForAWeek
        );
      ...
      if (weeklyQuota && weeklyQuota > 0) {
        ...
        weeklyRemainingLimit =
          weeklyQuota - totalBookedSeat;
  ...
  cancelBooking: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "cancelled!", data: cancelled });
    } catch (err) {
      res.status(500).json({
        errors: [
          {
