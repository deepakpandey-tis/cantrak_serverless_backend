...
const getTotalPlants = async (req, res) => {
    try {
 ...
 */
        sqlSelect = `SELECT count(DISTINCT p."plantSerial") "totalPlants"
        `;
        ...
        AND pl.id = p."plantLotId" AND p.id = pl2."plantId"
        `;
        ...
        return res.status(200).json({
            ...
            message: "Plants Count!"
        });

    } catch (err) {
        console.log("[controllers][dashboard][plants][getTotalPlants] :  Error", err);
