...
const getStorageLocationReceiptRegisterExcel = async (req, res) => {
    try {
        ...
        , tt."nameEn" "txnTypeEn", tt."nameTh" "txnTypeTh", to_char(to_timestamp(it.date/1000), 'DD/MM/YYYY') date, it."txnId", it."lotNo", "quantity"
        `;
        ...
        var tempFilePath = path.join(global.appRoot, 'tmp', 'receipt_register' + '.xlsx'); // 'tmp/' + msisdn + '.xlsx';                  // 'tmp/MSISDN2021.xlsx';     // on cloud '/tmp/MSISDN2021.xlsx'
