...
const getItemTxn = async (req, res) => {
    try {
        ...
        const { txnId, subId } = req.body;

        let sqlStr, sqlSelect, sqlFrom, sqlWhere, sqlOrderBy;

        const schema = Joi.object().keys({
            txnId: Joi.number().required(),
        ...
        sqlWhere = ` WHERE it."txnId" = ${payload.txnId} AND it."subId" = ${payload.subId} AND it."orgId" = ${orgId}`;
