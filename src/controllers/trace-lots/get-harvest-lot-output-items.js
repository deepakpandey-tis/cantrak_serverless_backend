...
const getHarvestLotOutputItems = async (req, res) => {
    try {
        ...
        sqlStr = `SELECT hpl.id "lotId", it.id "itemTxnId", it.quantity, it."expiryDate"
        ...
        FROM harvest_plant_lots hpl, item_txns it, items i, ums, species s, strains s2
        WHERE hpl."orgId" = ${orgId} AND hpl."companyId" = ${payload.companyId} AND hpl."id" = '${payload.id}'
        AND it."harvestPlantLotId" = hpl.id AND it."txnType" = ${TxnTypes.ReceiveProductFromHarvest} AND it.quantity > 0
        ...
        console.log("[controllers][trace-lots][getHarvestLotOutputItems] :  Error", err);
...
module.exports = getHarvestLotOutputItems;
