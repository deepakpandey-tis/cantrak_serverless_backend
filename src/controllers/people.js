        ...
        try {
            ...
            await knex.transaction(async(trx) => {
                ...
                const schema = Joi.object().keys({
                    ...
                    phoneNo: Joi.string().allow(null).allow("").optional()
                    ...
                    for (let peopleData of data) {
                        ...
                        if (i > 1) {
                            ...
                            if (peopleData.F) {
                                ...
                                let checkUser = await knex('users').select("id")
                                    .where({ userName: peopleData.F })

                                if (checkUser.length) {

                                    let values = _.values(peopleData)
                                    values.unshift('User Name already exists')
                                    errors.push(values);
                                    fail++;
                                    continue;
                                }
    ...
    updatePeople: async(req, res) => {
        try {
            ...
            await knex.transaction(async trx => {
                ...
                let existMobile;
                if (payload.mobileNo) {
                    existMobile = await knex('users').where({ mobileNo: payload.mobileNo });
                }
