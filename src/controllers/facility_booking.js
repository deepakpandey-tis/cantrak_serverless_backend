...
const facilityBookingController = {
    addFacility: async (req, res) => {
        try {
            ...
            await knex.transaction(async (trx) => {
                ...
                let bookingAllowOpenTime;
                let bookingAllowCloseTime;
                if (req.body.statuses.alwaysAllow == true) {
                    //  bookingAllowOpenTime = "";
                    //bookingAllowCloseTime = "";
                } else {

                    bookingAllowOpenTime = bookingCriteriaPayload.bookingAllowedAdvanceTime;
                    bookingAllowCloseTime = bookingCriteriaPayload.bookingCloseAdvanceTime;
                ...
                addedBookingCriteriaResult = await knex("entity_booking_criteria")
                    .insert({
                        ...
                        createdAt: currentTime,
                        orgId: req.orgId,
                        bookingAllowedAdvanceTime: bookingAllowOpenTime,
                        bookingCloseAdvanceTime: bookingAllowCloseTime,
                    })
                    .returning(["*"]);
            ...
            let [
                ...
                knex("facility_property_unit_type_quota_limit")
                    ...
                    .where({
                        ...
                        "facility_property_unit_type_quota_limit.entityType": "facility_master",
    ...
    facilityListingByDate: async (req, res) => {
        try {
            ...
            console.log("facility report data", payload)
            let {
                companyId,
                projectId,
                buildingPhaseId,
            ...
            if (
                ...
                facilityReportResult = await knex
                    ...
                    .whereBetween("entity_bookings.bookingStartDateTime", [
                        bookingStartTime,
                        bookingEndTime,
                    ])
                    ...
                    .where((qb) => {
                        ...
                        if (status) {
                            ...
                            if (Status == "Approved,Pending,Cancelled,Confirmed") {
                                ...
                                qb.orWhere({ "entity_bookings.isBookingConfirmed": true, "entity_bookings.isBookingCancelled": false, "entity_bookings.confirmedType": 0 })
    ...
    getOrgUser: async (req, res) => {
        try {
            let userId = req.me.id;
            let userNameResult = await knex
                .from("users")
    ...
    cancelBooking: async (req, res) => {
        try {
            ...
            let orgMaster = await knex.from("organisations").where({ id: 53, organisationAdminId: 766 }).orWhere({ id: 89, organisationAdminId: 1188 }).first();
        ...
        try {
            ...
            if (
                ...
                let isValidBookingInsideAllowPeriod =
                    moment(currentTime).add(+bookingAllowingTiming.bookingAllowedAdvanceTime,
                        "minutes"
            ...
            if (
                ...
                let isValidBookingBeforeLockPeriod =
                    moment(currentTime).add(+bookingAllowingTiming.bookingCloseAdvanceTime,
                        "minutes"
                    ) < moment(bookingStartTime);
    ...
    ,
    /*GET PROPERTY UNIT BY FLOOR ID */
    getUnitByFloorId: async (req, res) => {
        try {
