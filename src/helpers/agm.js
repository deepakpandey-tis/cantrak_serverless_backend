...
const agmHelper = {
  ...
  generateVotingDocumentOnEFSv2: async ({ agmId, data, orgId, requestedBy }) => {
    ...
    try {
      ...
      // let agmPropertyUnitOwners = await knex('agm_owner_master').where({ agmId: agmId, eligibility: true })
      let agmPropertyUnitOwners = await knex.raw(`SELECT agm_owner_master."ownerGroupNo",(array_agg(agm_owner_master."ownerName"))[1] as "ownerName",array_agg(agm_owner_master."ownershipRatio") as "ownershipRatio",array_agg(agm_owner_master."unitId") as "unitId",array_agg(agm_owner_master."unitNumber") as "unitNumber",array_agg(agm_owner_master."id") as "id" from agm_owner_master where agm_owner_master."agmId" = ${agmId} GROUP BY (agm_owner_master."ownerGroupNo")`);
