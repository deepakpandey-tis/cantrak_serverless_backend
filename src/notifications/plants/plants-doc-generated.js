...
const plantsDocGeneratedNotification = {
    send:async(sender, receiver, data, allowedChannels = ALLOWED_CHANNELS)=>{
        try{
            console.log('[notifications][plants][plants-notification][send]: Data:', data);

            if (SHOULD_QUEUE) {
                await notification.queue(sender, receiver, JSON.parse(JSON.stringify(data)), allowedChannels, __filename);
                console.log('[notifications][plants][plants-notification][send]: All Notifications Queued');
            } else {
                await notification.send(sender, receiver, JSON.parse(JSON.stringify(data)), allowedChannels, plantsDocGeneratedNotification);
                console.log('[notifications][plants][plants-notification][send]: All Notifications Sent');
            }
    ...
    sendEmailNotification: async (sender, receiver, data) => {
        data = {
            ...
            template: 'agm/plants-doc-generated.ejs',
            templateData: {
            ...
            payload: {
                ...data,
                subject: 'Plants - QR Document Generated.',
            }
        };
...
module.exports = plantsDocGeneratedNotification