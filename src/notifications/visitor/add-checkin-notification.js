...
const addCheckinNotification = {
    send: async(data) => {
        try {
            console.log('[notifications][visitor][add-checkin-notification][send]: Data:', data);

            if (SHOULD_QUEUE) {
                await notification.queue(data.sender, data.receiver, JSON.parse(JSON.stringify(data.payload)), ALLOWED_CHANNELS, __filename);
                console.log('[notifications][visitor][add-checkin-notification][send]: All Notifications Queued');
            } else {
                await notification.send(data.sender, data.receiver, JSON.parse(JSON.stringify(data.payload)), ALLOWED_CHANNELS, addCheckinNotification);
                console.log('[notifications][visitor][add-checkin-notification][send]: All Notifications Sent');
            }
        } catch (err) {
            console.log('[notifications][visitor][add-checkin-notification][send]: Error ', err);
    ...
    sendInAppNotification: async (sender, receiver, payload) => {
        ...
        if(receiver.visitorStayover){
            body = `${receiver.visitorNames} has successfully checked-in for stayover on ${receiver.visitorArrivalDate} - ${receiver.visitorDepartureDate}.`;
        ...
        data = {
            ...
            payload: {
                subject: `Visitor Checked-in Notification`,
                body: body,
                subjectThai: `Visitor Checked-in Notification`,
                bodyThai: bodyThai,
                icon: icons,
                image: images,
                extraData: {
...
module.exports = addCheckinNotification;
