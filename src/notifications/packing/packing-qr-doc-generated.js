...
const packingQrDocGeneratedNotification = {
    send:async(sender, receiver, data, allowedChannels = ALLOWED_CHANNELS)=>{
        try{
            console.log('[notifications][packing][packing-qr-notification][send]: Data:', data);

            if (SHOULD_QUEUE) {
                await notification.queue(sender, receiver, JSON.parse(JSON.stringify(data)), allowedChannels, __filename);
                console.log('[notifications][packing][packing-qr-notification][send]: All Notifications Queued');
            } else {
                await notification.send(sender, receiver, JSON.parse(JSON.stringify(data)), allowedChannels, packingQrDocGeneratedNotification);
                console.log('[notifications][packing][packing-qr-notification][send]: All Notifications Sent');
            }
    ...
    sendEmailNotification: async (sender, receiver, data) => {
        data = {
            ...
            template: 'pdf-templates/packing-qr-template.ejs',
            templateData: {
            ...
            payload: {
                ...data,
                subject: 'Packing - QR Document Generated.',
            }
        };
    ...
    sendSocketNotification: async (sender, receiver, data) => {
        ...
        data = {
            ...
            senderId: sender.id,
            receiverId: receiver.id,
            channel: 'packing-qr-code-notification',
            payload: {
                subject: title,
                body: description + `from ${sender.name}`,
                icon: icons,
...
module.exports = packingQrDocGeneratedNotification